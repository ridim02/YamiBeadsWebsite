...
if (isset($_POST['buy'])) {
	...
	$sql_insert = "insert into `order`(`product_id`,`user_id`,`name`,`price`,`num_product`,`image`, `address`, `city`, `pincode`, `cust_name`, `mobile`, `email`, `date_time`, `payment`) select `product_id`,`user_id`,`name`,`price`,`num_product`,`image`, '$address', '$city', '$pincode', '$name', '$mobile', '$email', '$date_time', '$payment' from cart where `user_id`='$login_id'";
	...
	$sql_update1 = 'update `product` p inner join `cart` c on p.id = c.product_id set p.quantity=p.quantity-c.num_product where p.`id` in (select `product_id` from `cart` where `user_id` = '.$login_id.')';
	mysqli_query($conn, $sql_update1);

	$sql_update2 = "update `product` set `stock`='Out Of Stock' where `quantity` = 0";
